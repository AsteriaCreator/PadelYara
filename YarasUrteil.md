...
> **Status (2026-06-21):** Partially built and intentionally hidden.
>
> **What exists:**
> - Backend: scraper + analysis + verdict generator + `/api/urteil` (needs `GEMINI_API_KEY` on Railway to activate AI; without it returns raw stats with `ai_available: false`). Logs to `padel_checker.urteil_log`.
> - Frontend: **Spielanalyse** page at `/turnierjaeger/spielanalyse` (and `/:slug` for public profiles). Shows APN, Punkte, Platz, W/L stats, category progression chart, filterable partner stats table, full match history. "Kommt bald" placeholder for the AI verdict section.
> - Old `/urteil` route redirects to `/turnierjaeger/spielanalyse`.
>
> **What's hidden:** the SPIELANALYSE tab is removed from `TurnierjagerNav.tsx` TABS array so regular users can't find it. Still accessible via direct URL. Re-add the tab when ready to launch publicly.
>
> **To activate the full AI verdict:** set `GEMINI_API_KEY` (free Google AI Studio key) as a Railway env var and deploy.
>
> Companion to `PROJECT_STATE.md` and `Scrapers.md`. This is the source of truth for the player-analysis feature. Update it as decisions land.
...
### Output
- AI-written (Google Gemini free tier, Yara voice) grounded in computed stats, in **two distinct parts**
...
- **Verdict generation:** Google Gemini (free tier) with Yara's voice; feed it the computed stats and
