...
> and frontend (`/urteil` page) are in. Verdict is generated by **Google Gemini (free
> tier)** — no per-verdict cost. To go LIVE it needs a free `GEMINI_API_KEY` (Google AI
> Studio) set on Railway and a deploy. Without the key the endpoint still returns the
> facts, just no AI verdict (`ai_available: false`). Every search + verdict is logged to
> `padel_checker.urteil_log` (accountability). Name still provisional.
